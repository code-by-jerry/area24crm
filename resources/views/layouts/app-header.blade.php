    ...
    <div class="flex flex-col items-center justify-between grow xl:flex-row xl:px-6">
        ...
        <div :class="isApplicationMenuOpen ? 'flex' : 'hidden'"
            ...
            <div class="flex items-center gap-2 2xsm:gap-3">
                <!-- Vertical Switcher -->
                @php
                    $allVerticals   = collect(glob(config_path('verticals/*.php')))->map(fn($f) => require $f)->keyBy('slug');
                    $activeSlug     = session('active_vertical');
                    $activeVertical = $activeSlug ? ($allVerticals[$activeSlug] ?? null) : null;
                @endphp

                <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                    <button @click="open = !open"
                        class="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 h-11 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700 transition">
                        <span class="h-2 w-2 rounded-full flex-shrink-0"
                              style="background-color: {{ $activeVertical['color'] ?? '#94a3b8' }}"></span>
                        <span class="hidden sm:inline">{{ $activeVertical['name'] ?? 'Select Vertical' }}</span>
                    ...
                    <div x-show="open"
                         ...
                         class="absolute right-0 top-full mt-2 w-52 rounded-xl border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-gray-800 z-[99999] overflow-hidden">
                        ...
                        @foreach($allVerticals as $v)
                            <form method="POST" action="{{ route('vertical.switch') }}">
                                @csrf
                                <input type="hidden" name="vertical" value="{{ $v['slug'] }}" />
                                <button type="submit"
                                    class="flex w-full items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-gray-700/50 transition {{ $activeSlug === $v['slug'] ? 'font-semibold' : '' }}">
                                    <span class="h-2 w-2 rounded-full flex-shrink-0"
                                          style="background-color: {{ $v['color'] }}"></span>
                                    {{ $v['name'] }}
                                    @if($activeSlug === $v['slug'])
                                        <svg class="ml-auto h-4 w-4 text-brand-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
